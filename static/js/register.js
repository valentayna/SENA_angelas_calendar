var emailLabel = document.querySelector('#RegisterEmailLabel'),
	email = document.querySelector('#registerEmail'),
	unameLabel = document.querySelector('#RegisterNameLabel'),
	uname = document.querySelector('#registerName'),
	passwordLabel = document.querySelector('#RegisterPasswordLabel'),
...
var activeElement, curunameIndex, screenCenter, svgCoords, unameCoords, unameScrollMax, chinMin = .5, dFromC, mouthStatus = "small", blinking, eyeScale = 1, eyesCovered = false, showPasswordClicked = false;
...
//uname functions
function calculateFaceMove2(e) {
	var
		carPos = uname.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(uname),
		caretCoords = {}
		;
	if (carPos == null || carPos == 0) {
		// if browser doesn't support 'selectionEnd' property on input[type="uname"], use 'value.length' property instead
		carPos = uname.value.length;
	}
	[].forEach.call(copyStyle, function (prop) {
		div.style[prop] = copyStyle[prop];
	});
	div.style.position = 'absolute';
	document.body.appendChild(div);
	div.textContent = uname.value.substr(0, carPos);
	span.textContent = uname.value.substr(carPos) || '.';
	div.appendChild(span);
	div.style.visibility = 'hidden';


	if (uname.scrollWidth <= unameScrollMax) {
		caretCoords = getPosition(span);
		dFromC = screenCenter - (caretCoords.x + unameCoords.x);
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, unameCoords.x + caretCoords.x, unameCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, unameCoords.x + caretCoords.x, unameCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, unameCoords.x + caretCoords.x, unameCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, unameCoords.x + caretCoords.x, unameCoords.y + 25);
	} else {
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, unameCoords.x + unameScrollMax, unameCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, unameCoords.x + unameScrollMax, unameCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, unameCoords.x + unameScrollMax, unameCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, unameCoords.x + unameScrollMax, unameCoords.y + 25);
	}
...
function onunameInput(e) {
	calculateFaceMove2(e);
	var value = uname.value;
	curunameIndex = value.length;

	// very crude uname validation to trigger effects
	if (curunameIndex > 0) {
		if (mouthStatus == "small") {
			mouthStatus = "medium";
			TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 1, { morphSVG: mouthMediumBG, shapeIndex: 8, ease: Expo.easeOut });
...
function onunameBlur(e) {
	activeElement = null;
	setTimeout(function () {
		if (activeElement == "uname") {
		} else {
			if (e.target.value == "") {
				e.target.parentElement.classList.remove("focusWithText");
			}
...
function initLoginForm() {
	...
	//manejar eventos para uname input
	uname.addEventListener('focus', onunameFocus);
	uname.addEventListener('blur', onunameBlur);
	uname.addEventListener('input', onunameInput);
	unameLabel.addEventListener('click', onunameLabelClick);
	...
	unameScrollMax = uname.scrollWidth;
